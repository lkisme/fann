...
FANN_EXTERNAL void FANN_API fann_cascadetrain_on_data(struct fann *ann,
  ...
  for (i = 1; i <= max_neurons; i++) {
    ...
    /* print current error */
    if (neurons_between_reports && (i % neurons_between_reports == 0 || i == max_neurons ||
                                    i == 1 || desired_error_reached == 0)) {
      if (ann->callback == NULL) {
        ...
        if ((ann->last_layer - 2) != ann->first_layer) {
          printf(
              ". candidate steepness %.2f. function %s",
        ...
        printf("\n");
      } else if ((*ann->callback)(ann, data, max_neurons, neurons_between_reports, desired_error,
                                  total_epochs) == -1) {
...
int fann_train_outputs(struct fann *ann, struct fann_train_data *data, float desired_error) {
  ...
  if (fann_desired_error_reached(ann, desired_error) == 0) return 1;
  ...
  for (i = 1; i < max_epochs; i++) {
    ...
    if ((target_improvement >= 0 &&
       ...
       * stagnation=%d\n", error_improvement, target_improvement, backslide_improvement,
       * stagnation); */
...
float fann_train_outputs_epoch(struct fann *ann, struct fann_train_data *data) {
  ...
  switch (ann->training_algorithm) {
    ...
    case FANN_TRAIN_SARPROP:
      fann_update_weights_sarprop(ann, ann->sarprop_epoch,
...
#endif
  ann->connections = (struct fann_neuron **)realloc(
      ann->connections, total_connections * sizeof(struct fann_neuron *));
...
int fann_reallocate_neurons(struct fann *ann, unsigned int total_neurons) {
  ...
  neurons = (struct fann_neuron *)realloc(ann->first_layer->first_neuron,
...
void initialize_candidate_weights(struct fann *ann, unsigned int first_con, unsigned int last_con,
                                  float scale_factor) {
  fann_type prev_step;
  unsigned int i = 0;
  unsigned int bias_weight =
      (unsigned int)(first_con + (ann->first_layer->last_neuron - ann->first_layer->first_neuron) -
                     1);
    ...
    for (j = 0; j < ann->cascade_activation_steepnesses_count; j++) {
      for (k = 0; k < ann->cascade_num_candidate_groups; k++) {
        ...
        initialize_candidate_weights(ann, neurons[candidate_index].first_con,
                                     neurons[candidate_index].last_con + candidate_connections_out,
                                     scale_factor);
        candidate_index++;
      }
    }
  ...
  for (i = 0; i < max_epochs; i++) {
    ...
    if ((best_cand_score > target_cand_score) || (best_cand_score < backslide_cand_score)) {
      ...
      printf("Best candidate score %f, real score: %f\n", ann->MSE_value - best_cand_score,
             best_cand_score);
      /* printf("best_cand_score=%f, target_cand_score=%f, backslide_cand_score=%f,
...
void fann_update_candidate_slopes(struct fann *ann) {
  ...
  for (cand_it = first_cand; cand_it < last_cand; cand_it++) {
    ...
    for (; i != num_connections; i += 4) {
      cand_sum += weights[i] * neurons[i].value + weights[i + 1] * neurons[i + 1].value +
    ...
    activation =
        fann_activation(ann, cand_it->activation_function, cand_it->activation_steepness, cand_sum);
    /* printf("%f = sigmoid(%f);\n", activation, cand_sum); */

    cand_it->sum = cand_sum;
    cand_it->value = activation;

    derived = fann_activation_derived(cand_it->activation_function, cand_it->activation_steepness,
                                      activation, cand_sum);
...
void fann_update_candidate_weights(struct fann *ann, unsigned int num_data) {
  struct fann_neuron *first_cand =
      (ann->last_layer - 1)->last_neuron +
      1; /* there is an empty neuron between the actual neurons and the candidate neuron */
...
fann_type fann_train_candidates_epoch(struct fann *ann, struct fann_train_data *data) {
  ...
  for (i = 0; i < data->num_data; i++) {
    ...
    for (j = 0; j < ann->num_output; j++) {
       ...
       * output_train_errors[j], (ann->output[j] - data->output[i][j]), output_train_errors[j],
       * ann->output[j], data->output[i][j]);
       * } */
...
void fann_add_candidate_neuron(struct fann *ann, struct fann_layer *layer) {
  unsigned int num_connections_in =
      (unsigned int)(layer->first_neuron - ann->first_layer->first_neuron);
  unsigned int num_connections_out =
      (unsigned int)((ann->last_layer - 1)->last_neuron - (layer + 1)->first_neuron);
  ...
  for (layer_it = ann->last_layer - 1; layer_it != layer; layer_it--) {
    ...
    printf("move neuron pointers in layer %d, first(%d -> %d), last(%d -> %d)\n",
           layer_it - ann->first_layer, layer_it->first_neuron - ann->first_layer->first_neuron,
...
#ifdef CASCADE_DEBUG_FULL
    printf("move weights[%d] -> weights[%d] (%f)\n", i + candidate_con, i + neuron_place->first_con,
           ann->weights[i + neuron_place->first_con]);
#endif
  }
...
FANN_EXTERNAL unsigned int FANN_API fann_get_cascade_num_candidates(struct fann *ann) {
  return ann->cascade_activation_functions_count * ann->cascade_activation_steepnesses_count *
...
FANN_EXTERNAL void FANN_API fann_set_cascade_activation_functions(
    struct fann *ann, enum fann_activationfunc_enum *cascade_activation_functions,
...
fann_set_cascade_activation_steepnesses(struct fann *ann, fann_type *cascade_activation_steepnesses,
