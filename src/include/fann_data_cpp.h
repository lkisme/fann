...
/* Section: FANN C++ Datatypes
   This section includes enums and helper data types used by the two main classes <neural_net> and
   <training_data>
 */
...
*/
enum error_function_enum { ERRORFUNC_LINEAR = FANN_ERRORFUNC_LINEAR, ERRORFUNC_TANH };
...
*/
enum stop_function_enum { STOPFUNC_MSE = FANN_STOPFUNC_MSE, STOPFUNC_BIT };
...
/* Enum: training_algorithm_enum
   ...
   with this algorithm, while other more advanced problems will not train very well. TRAIN_BATCH -
   Standard backpropagation algorithm, where the weights are updated after calculating the mean
   square error for the whole training set. This means that the weights are only updated once during
   an epoch. For this reason some problems, will train slower with this algorithm. But since the
   mean square error is calculated more correctly than in incremental training, some problems will
   reach a better solutions with this algorithm. TRAIN_RPROP - A more advanced batch training
   algorithm which achieves good results for many problems. The RPROP training algorithm is
   adaptive, and does therefore not use the learning_rate. Some other parameters can however be set
   to change the way the RPROP algorithm works, but it is only recommended for users with insight in
   how the RPROP training algorithm works. The RPROP training algorithm is described by [Riedmiller
   and Braun, 1993], but the actual learning algorithm used here is the iRPROP- training algorithm
   which is described by [Igel and Husken, 2000] which is a variant of the standard RPROP training
   algorithm. TRAIN_QUICKPROP - A more advanced batch training algorithm which achieves good results
   ...
   these advanced parameters, for users with insight in how the quickprop training algorithm works.
            ...
            FANN_TRAIN_SARPROP - THE SARPROP ALGORITHM: A SIMULATED ANNEALING ENHANCEMENT TO
...
*/
enum network_type_enum { LAYER = FANN_NETTYPE_LAYER, SHORTCUT };
...
/* Type: callback_type
   ...
   <neural_net::train_on_file> or <neural_net::cascadetrain_on_data>.
    ...
    >    float desired_error, unsigned int epochs, void *user_data);
        ...
        The callback can be set by using <neural_net::set_callback> and is very useful for doing
   custom things during training. It is recommended to use this function when implementing custom
        training procedures, or when visualizing the training in a GUI etc. The parameters which the
        callback function takes is the parameters given to the <neural_net::train_on_data>, plus an
   epochs parameter which tells how many epochs the training have taken so far.
 ...
 */
typedef int (*callback_type)(neural_net &net, training_data &train, unsigned int max_epochs,
                             unsigned int epochs_between_reports, float desired_error,
                             unsigned int epochs, void *user_data);
}  // namespace FANN
